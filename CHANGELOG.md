...
- `celo-sepolia` is chain 11142220 and is published as `eip155:11142220`. Through 2.39.0 it was published as `eip155:44787`, which is Celo Alfajores, the testnet Celo Sepolia replaced; the RPCs the facilitator already used (`rpc.ankr.com/celo_sepolia` in production) answer `eth_chainId` = `0xaa044c` = 11142220. The v1 name `celo-sepolia` does not change, and no other network's entry in `/supported` changes: a test republishes production's 156-entry `/supported` through the new code and only celo-sepolia's CAIP-2 id differs.
- The Celo Sepolia USDC EIP-712 domain is `name` = `"USDC"`, `version` = `"2"`, chain id 11142220, as the contract `0x01C5…C44E` returns it. The static table said `"USD Coin"` with 44787, which matches nothing on-chain: its separator is `0x9a13…253d`, the contract's `DOMAIN_SEPARATOR()` is `0x23f4…9578`. A plain EOA signature is still judged by the token's own simulation; the static domain is what the EIP-6492 validator and DX402's payer-key recovery hash against.
- A v2 request naming `eip155:11142220` is now a celo-sepolia payment. Before, the conversion refused it as `Invalid CAIP-2 format`, so a v2 client naming the real chain could not pay.
- `eip155:44787` is not kept as an alias. `/verify` and `/settle` answer it with `400` and code `network_retired`, carrying `network` and `replacement` (`eip155:11142220`), in v1 and v2 and before the idempotency cache. `POST /accepts` keeps `reason: network_unknown` and names the replacement in `detail`. Neither SDK (npm 2.96.0, PyPI 0.88.0) sends 44787, and the discovery catalog held no resource on it.
