...
| **`upto`, `escrow` / `commerce`** | not on Arc | `/supported` lists `exact` and only `exact` for both Arc networks, and `UPTO_DEPLOYED_NETWORKS` (`src/upto/types.rs`) does not name Arc. A network being served for `exact` does not enable anything else |
| **ERC-8004** | served from 2.37.0: `/identity`, `/reputation`, `/register`, `POST /feedback`; rater-signed feedback on `arc` from 2.38.0 | `supported_networks()` (`src/erc8004/mod.rs`) names `arc` and `arc-testnet`, wired to the canonical registries. All six addresses were read on 2026-09-22 against `rpc.mainnet.arc.io` and `rpc.testnet.arc.io`: 130-byte proxies whose implementation is the one Base and Base Sepolia run, `getVersion()` = `2.0.0`. The facilitator pays the gas in USDC. **Rater-signed feedback (`/feedback/evm/prepare` + `/feedback/evm/submit`) is served on `arc`, not on `arc-testnet`.** On mainnet it goes through Execution Market's v4 `FeedbackDelegate` `0x955Cc9fB9aB95FC0821ae74197D273dde5dA84f1`, read on 2026-09-23 against `rpc.mainnet.arc.io` and a second RPC: 5857 bytes, `VERSION()` = 4, `REPUTATION_REGISTRY()` = the mainnet registry above; the facilitator re-checks the code, the pinned registry and the version (ERC-165) on every request. No delegate is deployed on `arc-testnet`, so there the facilitator is the author of a rating |
