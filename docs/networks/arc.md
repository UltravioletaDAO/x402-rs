...
| **ERC-8004** | served from 2.37.0: `/identity`, `/reputation`, `/register`, `POST /feedback` | `supported_networks()` (`src/erc8004/mod.rs`) names `arc` and `arc-testnet`, wired to the canonical registries. All six addresses were read on 2026-09-22 against `rpc.mainnet.arc.io` and `rpc.testnet.arc.io`: 130-byte proxies whose implementation is the one Base and Base Sepolia run, `getVersion()` = `2.0.0`. The facilitator pays the gas in USDC. **Rater-signed feedback (`/feedback/evm/prepare`) is not on Arc**: no feedback delegate is deployed there, so on Arc the facilitator is the author of a rating |
