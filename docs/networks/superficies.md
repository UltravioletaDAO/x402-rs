...
| Página `/networks`, tabla principal y tabla por familia | `static/networks.html` | **generadas** desde `/supported`; íconos y familia desde `/networks.json` (2.41.0: se fue la regex que mandaba a EVM lo que no reconocía) | sí | `tests/frontend-capabilities.test.cjs` |
| `/networks`, `PUBLIC_RPCS` | `static/networks.html` | **a mano**, 39 claves, sin Arc ni Hedera (hay respaldo detrás de `/api/balances`) | sí | nada |
| `/networks`, tabla de wallets | `static/networks.html` | **generada** del `feePayer` de `/supported` para toda red que lo publica (Solana, Fogo, NEAR, Stellar, Algorand, Sui, Hedera); **a mano** solo EVM (DeBank) y XRPL, que no publican ninguno; exploradores de `/networks.json` | sí | `static_types_no_explorer_and_no_icon` (ningún explorador tipeado) |
