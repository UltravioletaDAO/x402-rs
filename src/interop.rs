...
//!
//! - `rate_limits` is every governor the router mounted, read from
//!   [`crate::rate_limit::mounted`] -- the same [`crate::rate_limit::Policy`]
//!   that configures each limiter and stamps `RateLimit-Policy` on its
//!   responses. A limit added to the router appears here without anyone
//!   editing this file.
...
use crate::rate_limit::{Door, Mount};
...
fn rate_limits(mounts: &[Mount]) -> Vec<Value> {
    ...
    let mut out = Vec::new();
    for mount in mounts {
        let key = (mount.door, mount.policy.quota(), mount.policy.window_s());
...
mod tests {
    ...
    /// The limits `main.rs` and `handlers` mount, with the numbers they mount
    /// today (the env-driven ones at their defaults), on their doors.
    fn production_like_mounts() -> Vec<Mount> {
        let m = |name, ms, burst, door| Mount {
            policy: Policy::new(name, Duration::from_millis(ms), burst),
            door,
        };
        vec![
            m("verify-settle", 2_000, 30, Door::Api),
            m("verify-settle", 2_000, 30, Door::Mcp),
            m("discovery-register", 12_000, 250, Door::Api),
            m("identity-read", 500, 60, Door::Api),
            m("secondary-read", 300, 100, Door::Api),
            m("erc8004-writes", 12_000, 30, Door::Api),
            m("discovery-read", 200, 120, Door::Api),
            m("human-pages", 500, 60, Door::Api),
            m("events", 2_000, 10, Door::Api),
        ]
    }
